...
★ テンプレートフォルダ <br>
Django のテンプレートシステムを使って，ビューから使用できるテンプレートを作成し，Python からデザインを分離する．
最初はアプリに，「templates」ディレクトリがないのでディレクトリを作成する．Django はこの中からテンプレートを探す．
テンプレートの配置は，一般的に，その下の階層にアプリケーションの名前のディレクトリを作成してから，その中に html ファイルを配置する構成になっている．
...
## Django チュートリアルが章ごとに,どんなことをしているのかの簡単な説明
...
- 初めての Django アプリ作成 3<br>
  Django はビューをリクエストされた URL から決定する．Django は「URLConf」と呼ばれているものを使う．URL パターンをビューにマッピングするもの．URLConf の基本的な使い方について．
- 初めての Django アプリ作成 4<br>
  簡単なフォーム処理とコードの縮小化を行う．
